...
describe("generatePlayoffs", () => {
  ...
  test("assigns court + scheduledAt to every playoff match", () => {
    const st = E.generatePlayoffs(fullGroupState());
    const byId = Object.fromEntries(st.matches.map((m) => [m.id, m]));
    const [qf1, qf2, qf3, qf4] = st.playoffs.bracket.qf.map((id) => byId[id]);
    assert.deepEqual({ c: qf1.court, t: qf1.scheduledAt }, { c: "1", t: "14:00" });
    assert.deepEqual({ c: qf2.court, t: qf2.scheduledAt }, { c: "2", t: "14:00" });
    assert.deepEqual({ c: qf3.court, t: qf3.scheduledAt }, { c: "1", t: "15:15" });
    assert.deepEqual({ c: qf4.court, t: qf4.scheduledAt }, { c: "2", t: "15:15" });

    const [sf1, sf2] = st.playoffs.bracket.sf.map((id) => byId[id]);
    assert.deepEqual({ c: sf1.court, t: sf1.scheduledAt }, { c: "1", t: "16:30" });
    assert.deepEqual({ c: sf2.court, t: sf2.scheduledAt }, { c: "2", t: "16:30" });

    const fin = byId[st.playoffs.bracket.final];
    const third = byId[st.playoffs.bracket.third];
    assert.deepEqual({ c: fin.court, t: fin.scheduledAt }, { c: "1", t: "17:15" });
    assert.deepEqual({ c: third.court, t: third.scheduledAt }, { c: "2", t: "17:15" });

    const [p9a, p9b] = st.playoffs.bracket.place9.map((id) => byId[id]);
    assert.deepEqual({ c: p9a.court, t: p9a.scheduledAt }, { c: "3", t: "14:00" });
    assert.deepEqual({ c: p9b.court, t: p9b.scheduledAt }, { c: "3", t: "15:15" });
  });
});

describe("maybeAutoGeneratePlayoffs", () => {
  function groupStateWithOneUnconfirmed() {
    const groups = ["A", "B", "C", "D"];
    const teams = [];
    ...
    groups.forEach((g, gi) => {
      matches.push(
        makeMatch({ group: g, stage: "group",
          status: (gi === 0 ? "live" : "confirmed"), // one unfinished match in group A
          teamAId: `${g}1`, teamBId: `${g}2`,
          sets: [{ a: 25, b: 0 }, { a: 25, b: 0 }, { a: 0, b: 0 }] }),
        makeMatch({ group: g, stage: "group", status: "confirmed",
          teamAId: `${g}1`, teamBId: `${g}3`,
          sets: [{ a: 25, b: 0 }, { a: 25, b: 0 }, { a: 0, b: 0 }] }),
        makeMatch({ group: g, stage: "group", status: "confirmed",
          teamAId: `${g}2`, teamBId: `${g}3`,
          sets: [{ a: 25, b: 0 }, { a: 25, b: 0 }, { a: 0, b: 0 }] }),
      );
    });
    return { teams, matches };
  ...
  function fullyConfirmedGroupState() {
    const st = groupStateWithOneUnconfirmed();
    st.matches = st.matches.map(m => ({ ...m, status: "confirmed" }));
    return st;
  }

  test("returns state unchanged when some group matches are still unconfirmed", () => {
    const before = groupStateWithOneUnconfirmed();
    const after = E.maybeAutoGeneratePlayoffs(before);
    assert.equal(after.playoffs?.generated, undefined);
    assert.equal((after.matches || []).filter(m => m.stage === "quarterfinal").length, 0);
  });

  test("auto-generates full bracket once every group match is confirmed", () => {
    const before = fullyConfirmedGroupState();
    const after = E.maybeAutoGeneratePlayoffs(before);
    assert.equal(after.playoffs.generated, true);
    assert.equal(after.playoffs.autoGenerated, true);
    assert.equal(after.playoffs.bracket.qf.length, 4);
    assert.equal(after.playoffs.bracket.sf.length, 2);
    assert.ok(after.playoffs.bracket.final);
    assert.ok(after.playoffs.bracket.third);
    assert.equal(after.playoffs.bracket.place9.length, 2);
  });

  test("is a no-op when playoffs have already been generated", () => {
    const pre = E.generatePlayoffs(fullyConfirmedGroupState());
    const initialMatchCount = pre.matches.length;
    const after = E.maybeAutoGeneratePlayoffs(pre);
    assert.equal(after.matches.length, initialMatchCount);
    // Second call must not create a duplicate autoGenerated flag if playoffs exist
    assert.equal(after.playoffs.generated, true);
  });

  test("returns state unchanged when there are no group matches at all", () => {
    const before = { teams: [], matches: [] };
    const after = E.maybeAutoGeneratePlayoffs(before);
    assert.equal(after.playoffs?.generated, undefined);
  });
});
